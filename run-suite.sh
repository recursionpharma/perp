...
docker run -e PY_VERSION=$py_version $docker_image -c "/test/bootstrap-pipenv.sh 1>&2 && cat Pipfile.lock" > $project_lock_dir/Pipfile.lock 2> $project_logs_dir/create-lock-pipenv.log
echo "return code: $?" >> $project_logs_dir/create-lock-pipenv.log

docker run -e PY_VERSION=$py_version $docker_image -c "/test/bootstrap-poetry.sh 1>&2 && cat poetry.lock" > $project_lock_dir/poetry.lock 2> $project_logs_dir/create-lock-poetry.log
echo "return code: $?" >> $project_logs_dir/create-lock-poetry.log

docker run -e PY_VERSION=$py_version $docker_image -c "/test/bootstrap-pip-compile.sh 1>&2 && cat requirements.txt" > $project_lock_dir/requirements.txt 2> $project_logs_dir/create-lock-pip.log
